 ...
 * Plugin URI: https://yoursite.com/wc-all-cart-tracker
...
class WC_All_Cart_Tracker
{
    ...
    private function get_analytics_data($days = 30)
    {
        ...
        // Converted carts (inactive) in period
        $converted_carts = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$table_name} WHERE last_updated >= %s AND is_active = 0",
            $date_from
        ));
        ...
        $avg_converted_cart = $wpdb->get_var($wpdb->prepare(
            "SELECT AVG(cart_total) FROM {$table_name} WHERE last_updated >= %s AND is_active = 0",
            $date_from
        ));
        ...
        // Abandonment rate
        $abandonment_rate = $total_carts > 0 ? (($abandoned_carts + $active_carts) / $total_carts) * 100 : 0;
        ...
        $registered_converted = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$table_name} WHERE last_updated >= %s AND user_id > 0 AND is_active = 0",
            $date_from
        ));
